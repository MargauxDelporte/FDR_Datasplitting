...
source(paste0(mywd,'/Functions/SVM_HDNL.R'))
...
Compare_SignalStrength <- function(rho) {
  set.seed(1)
  delta <-10
  # signal_index <- sample(c(1:p), size = p0, replace = F)
  # simulate data
  ...
  diag(S) <- 1 
  W <- rWishart(n=1, df=p+5, Sigma=S)[,,1]
  X1 <- mvrnorm(n1,mu=rep(-0.025,p),Sigma=W) #mu=rep(0,p)
  X2 <- mvrnorm(n2,mu=rep(0.025,p),Sigma=W) #mu=rep(0,p)
  X  <- rbind(X1, X2)
  beta_star <- numeric(p)
  ...
  g1 <- ApplySVM( X = X, y = y, q = q, num_split = num_split,myparms=params,
  ...
  ResultsDataFrame <- rbind(
    ResultsDataFrame,
    data.frame(Method = "Boost DS",                Delta = rho, FDP = g1$DS_fdp,    Power = g1$DS_power),
    data.frame(Method = "Boost MS",                Delta = rho, FDP = g1$MDS_fdp,   Power = g1$MDS_power)
  )
  return(ResultsDataFrame)
}

Compare_SignalStrength(0)

#######run the code#############
Results=data.frame()
for(s in 1:25){
  for(rho in seq(from=0,to=0.9,by=0.1)){
...
source(file.path(mywd, 'Functions', 'SVM_HDNL.R'))
...
pkgs <- c('e1071','kernlab','ranger','MASS','glmnet','knockoff','mvtnorm','hdi',
          'foreach','doParallel')
lapply(pkgs, library, character.only = TRUE)

# === PARAMETER GRID ===
param_grid <- expand.grid(
  i = seq(from = 0, to = 0.9, by = 0.1)
)
...
clusterEvalQ(cl, {
  ...
  source(file.path(mywd, 'Functions Dai', 'DS.R'))
  source(file.path(mywd, 'Functions Dai', 'fdp_power.R'))
  lapply(c('kernlab','e1071','ranger','MASS','glmnet','knockoff','mvtnorm','hdi'),
...
results_list <- foreach(
  ...
  chunk <- Compare_SignalStrength(rho = i_val)
...
PowerPlot <- ggplot(resultsagg, aes(x = SignalStrength, y = as.numeric(Avg_Power), color = Method)) +
  geom_point(size = 3) +
  geom_line()+
  labs(x = "Rho", y = "Power") +
...
FDRPlot=ggplot(resultsagg, aes(x = SignalStrength, y = as.numeric(Avg_FDR ), color = Method)) +
