...
param_grid_red=param_grid
# 0.6592501
# param_grid=param_grid[1:5,]
# helper
...
# parallel loop j=1
results <- foreach(j = seq_len(nrow(param_grid_red)), .combine = rbind, 
                   .packages = "xgboost") %dopar% {
                     
                     pars   <- as.list(param_grid_red[j, c("eta","max_depth",
                                                           # "min_child_weight",
                                                           # "gamma",
                                                           #"subsample",
                                                           #"colsample_bytree",
...
param_grid_red$mean_R2[results[,"j"]] <- results[,"mean_R2"]
...
# inspect top configs
head(param_grid_red[order(-param_grid_red$mean_R2), ], 5)

param_grid_red
param_grid_full
#param_grid_full=c()
param_grid_full=param_grid_red
#write.csv(param_grid_full,file='param_grid_full.csv')
nrow(param_grid_full)
#View(param_grid_full)
getwd()
selected=which(param_grid_full$mean_R2==max(param_grid_full$mean_R2))
param_grid_full[selected,]
