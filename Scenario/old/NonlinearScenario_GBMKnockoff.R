...
Compare_SignalStrength <- function(i, s) {
  ...
  knockoff_result<- ApplyGBMKnockoff(    X = X, y = y, q = q)
...
Adapted_Knockoff<- function(i, s) {
  ...
  ResultsDataFrame <- rbind(
    ResultsDataFrame,
    data.frame(Method = "Knockoff",     
               Delta = i, FDP = knockoff_result$fdp, Power = knockoff_result$power)
  )
...
Results=data.frame()
for(s in 1:50){
  for(i in seq(from=7,to=13,by=1)){
  Results=rbind(Results,Adapted_Knockoff(i,s))
  print(s)
  }
  print(Results)
  }
...
#write.csv(Results,file='NonlinearScenario_Knockoff.csv')
Results2=read.csv("ResultsNonlinearScenario.csv")

Results2=subset(Results2,Results2$Method!='Knockoff')
MyResults=rbind(Results,Results2)
View(MyResults)
