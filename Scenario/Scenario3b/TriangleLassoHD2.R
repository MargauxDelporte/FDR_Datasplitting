...
ApplyTriangleLassoHD<-function(X, y, q,myseed=1,num_split=1,signal_index=signal_index,mylambda){
  set.seed(myseed)
  amountTrain=0.333
  ...
  # crossvalidation lasso myfactor=1
  #cv <- cv.glmnet(
  #  as.matrix(Xtrain),
    #  y[train_index],
  #  alpha = 1,
  ...
  best_lambda <- mylambda #cv$lambda.1se*myfactor
