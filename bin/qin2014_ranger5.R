  ...
  res_mat <- foreach(iter = 1:num_split,
                     ...
                     .packages = c("randomForest")) %dorng% {
                       ...
                       mynlm <- ranger(
                         formula        = y ~ .,
                         data           = dataTrain,
                         num.trees      = myntree,                          # can increase if needed
                         mtry           = mymtry,  # fewer vars per split
                         min.node.size  = mynodesize,                           # like nodesize
                         sample.fraction = 0.632,                       # bootstrap-style sampling
