...
# ==============================================================================
fit_rf <- function(mydata_full,#myntree,mymtry,mynodesize,
                   num_split=50, amountTrain=0.5, amountTest=0.5) {
  registerDoRNG(11272025) # Set seed for reproducibility
  set.seed(1272025)
  res_mat <- foreach(iter = 1:num_split,
                     .combine = "rbind",
                     .packages = c("ranger")) %dorng% {
                       ...
                       permR2 <- function(data, Y, j, model) {
                         ...
                         pred_perm <- predict(model, data = as.data.frame(Xperm))$predictions
                       ...
                       mynlm <- ranger(
                         formula        = y ~ .,
                         data           = dataTrain,
                         num.trees      = 2000,                          # can increase if needed
                         mtry           = 260,  # fewer vars per split
                         min.node.size  = 16,                           # like nodesize
                         sample.fraction = 0.9,                       # bootstrap-style sampling
                         splitrule      = "extratrees",                 # more random, good for low signal
                         importance     = "permutation",                # unbiased importance
                         respect.unordered.factors = "partition"
                       )
                       # --- R² on the two halves ---
                       # Prediction on sample_index1
                       pred1 <- predict(
                         mynlm,
                         data = as.data.frame(X[sample_index1, , drop = FALSE])
                       )$predictions
                       
                       # Prediction on sample_index2
                       pred2 <- predict(
                         mynlm,
                         data = as.data.frame(X[sample_index2, , drop = FALSE])
                       )$pr
                       ...
                       hist(mirror)
                       selected_index <- SelectFeatures(mirror, abs(mirror), q = 0)
...
fit_rf(mydata_full=mydata_full,num_split=2)
myresults=rep(0,6)
...
#finetune model
# ==============================================================================
check_rf <- function(mydata_full,myntree,mymtry,mynodesize, num_split=50, amountTrain=0.5, amountTest=0.5) {
  res_mat <- foreach(iter = 1:num_split,
                     .combine = "rbind",
  ...
  # ---- unpack ----
  R2orig1_vec    <- res_mat[, 1]
  R2orig2_vec    <- res_mat[, 2]
  
  return(c(myntree,mymtry,mynodesize,mean(R2orig1_vec),mean(R2orig2_vec)))
}
set.seed(1272025)
mydata_full=as.data.frame(cbind(y,X))
#fit_rf(mydata_full=mydata_full,myntree=10,mymtry=10,mynodesize=1,num_split=50)
myresults=rep(0,5)
for(i in seq(from=100,to=1000,by=250)){
  for(j in round(c(p/10,p/7,p/5,p/4,p/3,p/2))){
    for(k in seq(from=1,to=25,by=5)){
