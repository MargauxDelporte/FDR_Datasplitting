...
source(paste0(mywd,'/NoSplit/NoSplitLM.R'))
...
################################Triange: train test test each 30#################################### i=25
Compare_SignalStrenght=function(i,s){
  ...
  ###my own methods:
  g2=ApplyLMNoSplit(X=as.data.frame(X), y, q=0.1,num_split=10, signal_index=signal_index, amountTrain=0.5, myseed = 1)
...
resultsagg <- resultsagg %>%
  mutate(Method  = case_when(
    ...
    Method == "2 split DS" ~ "Delporte (single split)",
    Method == "2 splitMS" ~ "Delporte (10 splits)",
