...
Compare_SignalStrenght=function(i,s){
  ...
  ### randomly generate the true beta i=400
  beta_star <- rep(0, p)
  beta_star[signal_index] <- rnorm(p0, mean = 0, sd = delta*sqrt(log(p)/n))
  
  ### generate y
  y <- X^2%*%beta_star + rnorm(n, mean = 0, sd = 0.1)
  
  ###my own methods:
  g=ApplyTriangleBoostTrain(X=as.data.frame(X),mybooster='gbtree', y, q=0.1,num_split=10, signal_index=signal_index, amountTrain=0.333, myseed = 1)
...
Results=data.frame()
for(s in 1:25){
  for(i in seq(from=25,to=29,by=1)){
...
write.xlsx(Results,file='NonLinearScenario.xlsx')
