#j=1
#model=lm
permR2TriangleRangerTrain<-function(data,j,model){
  dataPerm<-data[,-1]
  dataPerm[,j]<-sample(data[,j+1],replace=FALSE)
  names(dataPerm)=paste0('X',1:p)
  predictLM<-predict(model,data=dataPerm)$predictions
  rsquared=1-sum((data$y-predictLM)^2)/sum((data$y-mean(data$y))^2)
  return(rsquared)
}

ApplyTriangleRangerTrain<-function(X, y, q,myseed=1,mybooster='gbtree',num_split=1,signal_index=signal_index,mylambda=1,myeta= 0.01){
  set.seed(myseed)
  amountTrain=0.333
  ...
  lm<- ranger(
    ...
    sample.fraction = 0.3,    # use 70% of the rows for each tree
  ...
  dataTest1<-data[sample_index1,]
  dataTest2<-data[sample_index2,]
  
  predict_TRAIN<-predict(lm, data = dataTrain)$predictions
  R2orig_TRAIN<-1-sum((y[train_index]-predict_TRAIN)^2)/sum((y[train_index]-mean(y[train_index]))^2)
  R2orig_TRAIN
  
  predictLM1<-predict(lm,data=dataTest1)$predictions
  predictLM2<-predict(lm,data=dataTest2)$predictions
  ...
  Rnew1<-sapply(1:ncol(X),function(j) permR2TriangleRangerTrain(data[sample_index1,],j,lm))
  Rnew2<-sapply(1:ncol(X),function(j) permR2TriangleRangerTrain(data[sample_index2,],j,lm))
  ...
  CalculateFDP_Power(selected_index, signal_index)
