#j=1
#model=lm
permR2ElastNetLassHD<-function(data,j,model){
  dataPerm<-data[,-1]
...
ApplyTriangleElastNetHD<-function(X, y, q,myseed=1,num_split=1,signal_index=signal_index){
  set.seed(myseed)
  amountTrain=0.333
  ...
  # Specify the Elastic Net mixing parameter (α between 0 = ridge and 1 = lasso)
  alpha_value <- 0.5
  
  # Cross‐validate for λ at fixed α
  cv_enet <- cv.glmnet(
    x     = as.matrix(Xtrain),
    y     = y[train_index],
    alpha = alpha_value
  ...
  # Extract the λ that gives minimum cross‐validated error
  best_lambda <- cv_enet$lambda.min
  
  # Fit the final Elastic Net model at (α, λ_best)
  lm <- glmnet(
    x      = as.matrix(Xtrain),
    y      = y[train_index],
    alpha  = alpha_value,
  ...
  Rnew1<-sapply(1:ncol(X),function(j) permR2ElastNetLassHD(data[sample_index1,],j,lm))
  Rnew2<-sapply(1:ncol(X),function(j) permR2ElastNetLassHD(data[sample_index2,],j,lm))
