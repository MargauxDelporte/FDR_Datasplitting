...
ApplyTriangleBoostTrain<-function(X, y, q,myseed=1,mybooster='gbtree',num_split=1,signal_index=signal_index){
  ...
  for(iter in 1:num_split){
    ...
    if (requireNamespace("energy", quietly = TRUE)) {
        scores <- sapply(Xtrain, function(z) energy::dcor(z, ytrain))
      } else {
        scores <- sapply(Xtrain, function(z) abs(suppressWarnings(cor(z^2, ytrain))))
      }
      ranked <- order(scores, decreasing = TRUE)
  
    keep_idx <- ranked[seq_len(min(50, ncol(Xtrain)))]
    keep_idx%in%signal_index
    Xtr <- Xtrain[, keep_idx, drop = FALSE]
    Xv  <- Xtrain[,   keep_idx, drop = FALSE]
    
    dat_tr <- data.frame(y = ytrain, Xtr)
    terms  <- paste(sprintf('s(%s, k=%d, bs="ts")', names(Xtr), 4), collapse = " + ")
