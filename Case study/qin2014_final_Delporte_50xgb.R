...
res_mat <- foreach(iter = 1:num_split,
                   ...
                   .packages = c("xgboost")) %dorng% {
                     ...
                     dtrain <- xgb.DMatrix(data = X_matrix[train_index,], label = y[train_index])
                     
                     mynlm <- xgb.train(params   = param,
                                      data     = dtrain,
                                      nrounds  = nrounds,
                                      verbose  = 0)

                     # --- R² on the two halves ---
                     pred1 <- predict(mynlm, newdata = as.data.frame(X_matrix[sample_index1, , drop = FALSE]))
                     pred2 <- predict(mynlm, newdata = as.data.frame(X_matrix[sample_index2, , drop = FALSE]))
