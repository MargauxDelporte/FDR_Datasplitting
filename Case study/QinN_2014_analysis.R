...
## Extract sample metadata + features
pd <- as.data.frame(colData(ra))
pd$sample_id <- rownames(pd)

X <- assay(ra)                    # predictors (taxa × samples)
X <- t(X)                         # now samples × taxa
tax_names <- rownames(assay(ra))  # original species names

## ----------------------------------------------------------
## Choose your continuous outcome
## ----------------------------------------------------------
# Check available metadata variables:
names(pd)
str(pd)

# Example: use age if available
cont_var <- "BMI"

# If "age" does NOT exist, pick another numeric variable:
# candidates <- names(Filter(is.numeric, pd))
# print(candidates)
...
hist(y)
table(pd$disease)
## Keep only samples with non-missing y
keep <- !is.na(y)
X <- X[keep, ]
y <- y[keep]
pd <- pd[keep, ]
...
# 2. Optionally drop extremely rare taxa
rare <- colMeans(X > 0) > 0.05   # present in >=5% samples
X <- X[, rare]

dim(X)   # number of samples × number of predictors
...
cat("Predictors:", ncol(dataset) - 2, "\n")  # subtract sample_id + y
