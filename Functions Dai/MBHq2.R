lm.pval <- function(x, y) {
  fit <- lm(y ~ x)
  pvals <- summary(fit)$coefficients[-1, "Pr(>|t|)"] ## drop intercept
  pvals
}

lm.ci <- function(x, y, level = 0.95, ...) {
  fit <- lm(y ~ x)
  confint(fit, level = level)[-1, , drop = FALSE] ## drop intercept row
}

multi.split <- function(x, y, B = 100, fraction = 0.5,
                        ci = TRUE, ci.level = 0.95,
                        model.selector = function(x, y, ...) {
                          fit <- cv.glmnet(x = x, y = y, family = "gaussian",
                                           type.measure = "mse", alpha = 1,
                                           nfolds = 25)
                          beta <- coef(fit, s = "lambda.1se")[-1, , drop = FALSE] ## drop intercept
                          sel <- which(beta != 0)   ## integer indices instead of names
                          sel
                        },
...
MBHq <- function(X, y, q, num_split){
  ...
  ## BY correction factor: c(p) = sum_{i=1}^p 1/i
  c_p <- sum(1 / (1:p))
  cutoff <- which(sorted_pvalues$x <= (1:p) * q / c_p)
